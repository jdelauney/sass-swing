...
export default class Slidify extends CustomComponent {
	constructor(element) {
		...
		// console.log(this.slideSizeInpx);
		// let test = -((0 * this.slideSizeInpx) + (this.slideSizeInpx / 2));
	...
	_createActionButton() {
		...
		let action = (this.options.autoplay) ? "stop" : "start";
	 ...
	 */
	static bind() {
		return Array.from(document.querySelectorAll("[data-js-slider]")).map(
			(element) => {
