...
export default class Modalify extends CustomComponent{
	...
	constructor(element) {
		...
		try {
			...
			this.buttons.forEach((btn) => {
				if ((btn.hasAttribute("value")) || (btn.hasAttribute("modal-result"))) {
	...
	_buttonHandlerClick(evt) {
		...
		else if (target.hasAttribute("modal-result")) {
			this._modalResult = target.getAttribute("modal-result");
		}
